...
sealed class LauncherForm : Form
{
    ...
    public LauncherForm()
    {
        Text="Frostbound Plus • 0.1.0"; ClientSize=new Size(760,430); MinimumSize=new Size(780,460); BackColor=Color.FromArgb(17,27,39); ForeColor=Color.White; Font=new Font("Segoe UI",11);
        ...
        Add("Choose client",Choose);Add("Download client",()=>Run(Install));Add("Register",()=>Account(false));Add("Recover account",()=>Account(true));Add("Refresh status",()=>Run(Refresh));Add("Play",()=>Run(Play));
        ...
        Shown+=(_,_)=>Run(Refresh);FormClosing+=(_,_)=>{operation?.Cancel();lifetime.Cancel();};
    }
    ...
    async Task Install()
    {
        ...
        var archive=Path.Combine(Services.StateDirectory,d.Sha256.ToLowerInvariant()+".zip.part");var staging=destination+".install-"+Guid.NewGuid().ToString("N");
        try
        {
            using var http=Services.CreateHttp();http.Timeout=Timeout.InfiniteTimeSpan;await Services.Download(http,d,archive,new Progress<string>(s=>status.Text=s),cts.Token);
            status.Text="Extracting verified client…";await Task.Run(()=>Services.ExtractZip(archive,staging,cts.Token),cts.Token);
