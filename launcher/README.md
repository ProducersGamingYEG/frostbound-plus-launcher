# Frostbound Plus launcher 0.1.1
...
`--self-test` makes no external calls or account changes. It writes a pass report beside the executable, or `self-test-error.txt` and exits 1. Google Drive share links use the download endpoint and supported confirmation forms. When sign-in, quota or unsupported confirmation prevents direct download, use **Open client source**, download in the browser, and choose **Install archive**. Local archives receive the same exact size/SHA-256 check before sequential safe extraction. RAR extraction uses SharpCompress 0.50.4 and rejects links, encrypted entries and split volumes.

The checks cover download resume, servers ignoring Range, corrupted checksum rejection, unsafe ZIP traversal, configuration preservation, missing client rejection and localhost URL rejection. Build outputs are not source.
