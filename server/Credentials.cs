...
{
    public static string User(string? value) => value is not null && Regex.IsMatch(value, "^[A-Za-z0-9_]{1,16}$") ? value.ToUpperInvariant() : throw new ArgumentException("Username must contain 1–16 letters, digits or underscores.");
    public static string Email(string? value)
    {
        var s = value?.Trim().ToLowerInvariant() ?? "";
        if (s.Length > 254 || !MailAddress.TryCreate(s, out var m) || m.Address != s || !s.Contains('@')) throw new ArgumentException("Enter a valid email address.");
        return s;
    }
    public static string Password(string? value) => value is not null && value.Length is >= 8 and <= 16 && value.All(c => c is >= '!' and <= '~') ? value.ToUpperInvariant() : throw new ArgumentException("Password must contain 8–16 printable ASCII characters without spaces.");
    public static (string Salt, string Verifier) Verifier(string username, string password, byte[]? salt = null)
    {
        salt ??= RandomNumberGenerator.GetBytes(32);
        if (salt.Length != 32) throw new ArgumentException("Salt must be 32 bytes.");
